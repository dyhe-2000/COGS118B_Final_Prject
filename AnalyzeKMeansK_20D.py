...
# every 10 is for a single run with 10 iterations
J_clust_list = np.loadtxt('J_clust_list_K_20_D_run_9.csv', delimiter=',')
...
# load C grouping for max and min runs
k_20_min_J_clust_run_C_recordings = np.loadtxt('C_K_20_D_run_' + str(min_J_clust_index) + '.csv', delimiter=',')
k_20_max_J_clust_run_C_recordings = np.loadtxt('C_K_20_D_run_' + str(max_J_clust_index) + '.csv', delimiter=',')
# print(k_20_min_J_clust_run_C_recordings.shape) # (60000,)
# print(k_20_max_J_clust_run_C_recordings.shape) # (60000,)

# load K Z vectors (the centroid) grouping for max and min runs
k_20_min_J_clust_run_Z_vectors = np.loadtxt('Kzs_K_20_D_run_' + str(min_J_clust_index) + '.csv', delimiter=',')
k_20_max_J_clust_run_Z_vectors = np.loadtxt('Kzs_K_20_D_run_' + str(max_J_clust_index) + '.csv', delimiter=',')
...
for i in range(20):
    ...
    for j in range(k_20_min_J_clust_run_C_recordings.shape[0]):
        if(k_20_min_J_clust_run_C_recordings[j] == i):
            min_norms_list_for_Zi.append((np.linalg.norm(k_20_min_J_clust_run_Z_vectors[i,:]-train_data_20_D_min[j,:]),j))
    for j in range(k_20_max_J_clust_run_C_recordings.shape[0]):
        if(k_20_max_J_clust_run_C_recordings[j] == i):
            max_norms_list_for_Zi.append((np.linalg.norm(k_20_max_J_clust_run_Z_vectors[i,:]-train_data_20_D_min[j,:]),j))
